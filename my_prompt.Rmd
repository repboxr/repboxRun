...
### source_with_log start: 2026-04-15 07:38:50 
...
Run  /home/rstudio/repbox/projects/aejapp_1_3_4/metareg/base/stata_code/mrb_stata.do
    finished after 0.91 seconds.

> mrb = mrb_run_r_base(mrb)

mrb_r_base processing runids: 106 108 110 112 114 117 119 121 123 127 130 132 136 138 141 143 206 Error in if (grepl("\n", file)) { : argument is of length zero
16: (function () 
    mrb_test_traceback())()
15: readr::datasource(file)
14: haven::read_dta(file_path) at t_merge.R#47
13: scmd_merge(data = data, merge_type = "1:1", varlist = "partner", 
        file_path = file.path(stata2r_env$working_dir, "met"), keep_opt = NA_character_, 
        has_nogenerate = FALSE) at <text>#1
...
8: mrb_run_r_base(mrb) at run_mrb_test.R#43
7: eval(ei, envir)
6: eval(ei, envir)
...
# FILE: test_runid_206.R
```
# Stata: merge partner using met
data = scmd_merge(data = data, merge_type = "1:1", varlist = "partner", file_path = file.path(stata2r_env$working_dir, "met"), keep_opt = NA_character_, has_nogenerate = FALSE)
...
data = scmd_generate(data = data, new_var = "hs", r_expr_str = "`real`(`product`)", is_string = FALSE, force_integer = FALSE)
...
# Stata: g lgdppcus_lmet=lgdppcus*lmet
data = scmd_generate(data = data, new_var = "lgdppcus_lmet", r_expr_str = "`lgdppcus`*`lmet`", is_string = FALSE, force_integer = FALSE)

# Stata: g corrupt_metdum=corrupt*metdum
data = scmd_generate(data = data, new_var = "corrupt_metdum", r_expr_str = "`corrupt`*`metdum`", is_string = FALSE, force_integer = FALSE)

# Stata: g corrupt_lmet=corrupt*lmet
data = scmd_generate(data = data, new_var = "corrupt_lmet", r_expr_str = "`corrupt`*`lmet`", is_string = FALSE, force_integer = FALSE)

# Stata: g lpop_metdum=lpop*metdum
data = scmd_generate(data = data, new_var = "lpop_metdum", r_expr_str = "`lpop`*`metdum`", is_string = FALSE, force_integer = FALSE)

# Stata: g lpop_lmet=lpop*lmet
data = scmd_generate(data = data, new_var = "lpop_lmet", r_expr_str = "`lpop`*`lmet`", is_string = FALSE, force_integer = FALSE)

# Stata: keep if corrupt~=.
data = scmd_keep(data = data, r_if_cond = "`corrupt`~=NA_real_") %>% 
  dplyr::mutate(stata2r_original_order_idx = dplyr::row_number())

# Stata: keep if lgdppcus~=.
data = scmd_keep(data = data, r_if_cond = "`lgdppcus`~=NA_real_") %>% 
  dplyr::mutate(stata2r_original_order_idx = dplyr::row_number())

# Stata: keep if lgap1~=.
data = scmd_keep(data = data, r_if_cond = "`lgap1`~=NA_real_") %>% 
  dplyr::mutate(stata2r_original_order_idx = dplyr::row_number())

# Stata: keep if hs4==9706
data = scmd_keep(data = data, r_if_cond = "`hs4`==9706") %>% 
  dplyr::mutate(stata2r_original_order_idx = dplyr::row_number())

# Stata: areg lgap1 corrupt lgdppcus metdum if reporter=="CHE",absorb(ry) cluster(partner)
data = stata2r::scmd_keep(data, r_if_cond = "`reporter`==\"CHE\"")
# Could not reconstruct R translation because required parcels are missing.

```
# END OF FILE: test_runid_206.R
